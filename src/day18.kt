...
into rows that are all the same width; you take note of the safe tiles (.) and traps (^) in the first row
(your puzzle input).

The type of tile (trapped or safe) in each row is based on the types of the tiles in the same position,
and to either side of that position, in the previous row. (If either side is off either end of the row, it counts
...
fun findNextRow(prevRow: String): String {
    var nextRow = ""
    var leftTrap = false
    var centerTrap = false
    var rightTrap = false

    val prevRowChars = prevRow.toCharArray()
    for (inx in 0..prevRowChars.size-1) {
        if (inx == 0) {
            leftTrap = false
        } else {
            leftTrap = centerTrap
        }
        centerTrap = (prevRowChars[inx] == '^')
        if (inx + 1 == prevRowChars.size) {
            rightTrap = false
        } else {
            rightTrap = (prevRowChars[inx + 1] == '^')
        }
        nextRow += ""
        /*
         ...
         */
        var isTrap = ((leftTrap && centerTrap && ! rightTrap) ||
                (centerTrap && rightTrap && ! leftTrap) ||
                (leftTrap && ! rightTrap && ! centerTrap) ||
                (rightTrap && ! leftTrap && ! centerTrap))
...
fun main(args: Array<String>) {
    ...
    val puzzleInput = ".^^^.^.^^^.^.......^^.^^^^.^^^^..^^^^^.^.^^^..^^.^.^^..^.^..^^...^.^^.^^^...^^.^.^^^..^^^^.....^...."
    //val puzzleInput = ".^^.^.^^^^"
    val rowList: MutableList<String> = mutableListOf(puzzleInput)

    (2..400000).forEach {
        rowList.add(findNextRow(rowList.last()))
    }

    var count = 0
    for (row in rowList) {
        count += row.toCharArray().filter{ it == '.'}.count()
    }
    rowList.forEach {
        println(it)
    }
