...
that you generate using a pre-agreed algorithm. Unfortunately, you've run out of keys in your one-time pad, and so you need to generate some more.
...
class BunnyKeyGenerator(val salt: String, val stretchCount :Int = 0) {
    val md5 = MD5Hasher()
    ...
    private fun generateHash(): String {
        var seed = (salt + counter.toString())
        for (x in 1..stretchCount) {
            val hash = md5.hash(seed)
            seed = hash
        }
        return seed
    }
...
fun main(args: Array<String>) {
    ...
    val bkg = BunnyKeyGenerator("jlmsuwbz")
    val numKeys = 64
    for (i in 1..numKeys) {
        val key = bkg.getNextKey()
        println("key ${i} is found at index ${bkg.index}")
    }

    println("----part 2-------")

    val bkg2 = BunnyKeyGenerator("jlmsuwbz", 2017)
    for (i in 1..numKeys) {
        val key = bkg2.getNextKey()
        println("key ${i} is found at index ${bkg2.index}")
    }

    // test answer should be 39 for abc
}